...
namespace micron {
    bool Client::CleanUSBBuffer()
    {
        ...
        flog::info("Cleaning USB buffer, have {} bytes to read", (int)bytesAvailable);
        ...
        {
            flog::error("CleanUSBBuffer() -- bytesAvailable > RECEIVE_BUFFER_SIZE");
            bytesAvailable = RECEIVE_BUFFER_SIZE;
        }

        DWORD bytesReceived;
        status = FT_Read(deviceHandle, g_receiveBuffer, bytesAvailable, &bytesReceived);
        if (status != FT_OK)
        {
